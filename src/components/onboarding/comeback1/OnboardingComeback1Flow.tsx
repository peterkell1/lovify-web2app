...
import {
  ...
  V3_FeatureChat,
  V3_Familiarity, V3_Proof1, V3_Proof2, V3_WhyBuilt, V3_SongIdeas, V3_Tracking, V3_Review, V3_Referral,
...
export function OnboardingComeback1Flow({ mode = 'app', startAt, offer }: { mode?: 'app' | 'web'; startAt?: string; offer?: string } = {}) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [songState, songs, state.savedVersion, visionUrl, sessionId]);
  ...
  const buyPlan = useCallback(async (planId: string, opts?: { email?: string }) => {
    ...
    if (mode === 'web') {
      ...
      // /offer: keep checkout ON-DOMAIN via RevenueCat's embedded Web Billing
      // SDK (Stripe Elements + Apple Pay) instead of the pay.rev.cat redirect.
      // Same RC App User ID (the onboarding session) → RC's webhook claims the
      // song + provisions the account, exactly like the redirect did. On success
