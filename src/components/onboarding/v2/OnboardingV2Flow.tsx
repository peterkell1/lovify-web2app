...
export function OnboardingV2Flow({ mode = 'app' }: { mode?: 'app' | 'web' } = {}) {
  ...
  useEffect(() => {
    if (!flowStartedRef.current) {
      ...
      capturePostHogEvent('onboarding_flow_started', { flow: 'onboarding_v2', surface, total_steps: TOTAL });
    }
    const stepId = stepIds[step] ?? `step_${step}`;
    capturePostHogEvent('onboarding_step_viewed', {
      flow: 'onboarding_v2', surface, step_id: stepId, step_index: step, total_steps: TOTAL,
    });
  ...
  const screen = useMemo(() => {
    ...
    switch (stepIds[step]) {
      ...
      case 'song_chat': return <V3_Chat
        ...
        onSave={() => { capturePostHogEvent('onboarding_song_created', { flow: 'onboarding_v2' }); next(); }}
        onBack={back}
      />;
      ...
      case 'create_account': return <V3_CreateAccount
        onApple={() => {
          ...
          capturePostHogEvent('onboarding_apple_handoff', { flow: 'onboarding_v2', surface });
        ...
        onContinueWithEmail={() => {
          ...
          capturePostHogEvent('onboarding_email_handoff', { flow: 'onboarding_v2', surface });
          const sid = sessionIdRef.current;
          clearSnapshot(mode);
          navigate(`/signup${sid ? `?fs=${encodeURIComponent(sid)}` : ''}`);
        }}
        onNext={async () => {
          capturePostHogEvent('onboarding_account_completed', { flow: 'onboarding_v2', surface });
